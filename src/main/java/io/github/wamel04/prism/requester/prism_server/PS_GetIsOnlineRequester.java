...
public class PS_GetIsOnlineRequester {
    ...
    public static CompletableFuture<Boolean> request(PrismServer server) {
        ...
        new Thread(() -> {
            try (Jedis jedis = PRISM.getJedis()) {
                ...
                jedis.publish("prism:ps_get_isonline_request", ProtocolMessageConvertor.convert(server.getServerName(), now));
                ...
                }, "prism:ps_get_isonline_receive_" + now);
            }
        }).start();
