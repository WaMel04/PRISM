...
public class PP_GetLocationRequester {
    ...
    public static CompletableFuture<String> request(PrismPlayer prismPlayer) {
        ...
        new Thread(() -> {
            try (Jedis jedis = PRISM.getJedis()) {
                jedis.publish("prism:pp_get_location_request", ProtocolMessageConvertor.convert(prismPlayer.getPrismServer().getServerName(), prismPlayer.getUuid().toString()));
                ...
                }, "prism:pp_get_location_receive_" + prismPlayer.getUuid().toString());
            }
        }).start();
