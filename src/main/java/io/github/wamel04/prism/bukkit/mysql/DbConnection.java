...
public class DbConnection {
    ...
    public DataSource dataSource(String username, String password) {
        ...
        config.setJdbcUrl("jdbc:mysql://" + MysqlConfig.ip + ":" + MysqlConfig.port + "/" + MysqlConfig.database + "?useUnicode=yes&characterEncoding=utf-8&useSSL=false");
    ...
    private void testConnection(HikariDataSource ds) {
        try (Connection ignored = ds.getConnection()) {
            Bukkit.getConsoleSender().sendMessage("§a[PRISM] 데이터베이스와 연결되었습니다. poolsize: " + ds.getMaximumPoolSize());
    ...
    public void closeConnection() {
        if (dataSource.isRunning() ||
            ...
            try {
                dataSource.close();
                Bukkit.getConsoleSender().sendMessage("§c[PRISM] 데이터베이스와의 연결을 종료합니다.");
            } catch (Exception e) {
                e.printStackTrace();
            }
    }
